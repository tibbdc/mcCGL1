...
def trans_model2enz_json_model_split_isoenzyme(model_file, reaction_kcat_mw_file, f, ptot, sigma, lowerbound, upperbound, json_output_file):
    ...
    * upperbound:  Upperbound  of enzyme concentration constraint. 
    ...
    json_path = "./analysis/%s_irreversible.json" % model_name
...
def optimize_enzyme_parameters(ETGEMs_model, expected_value, rounds_num, kcat_max_json):
    rounds = 1
    opt_etm = calculate_ETM(ETGEMs_model, "maximize", 0.0)
    ...
    def find_max_kcat_mw(ec_codes, max_kcat_dict):
        max_kcat = None  
        for ec_code in ec_codes:
            if ec_code in max_kcat_dict.keys():
                if max_kcat is None or max_kcat_dict[ec_code]["kcat_max"] > max_kcat:
                    max_kcat = max_kcat_dict[ec_code]["kcat_max"]
        return max_kcat

    for reaction in ETGEMs_model.reactions.values():
    ...
    while(opt_etm.obj() < expected_value and rounds < rounds_num):
        enzyme_usage_dict = {}
        for reaction_id in ETGEMs_model.reactions.keys():
            reaction = ETGEMs_model.get_reaction_by_id(reaction_id)
            enzyme_usage_dict[reaction_id] = opt_etm.reaction[reaction_id].value*reaction.mw/reaction.kcat
        ...
        max_enzyme_usage_reaction = ETGEMs_model.get_reaction_by_id(max_enzyme_usage_reaction_id)
        ec_code = max_enzyme_usage_reaction.annotation["ec-code"]
        max_enzyme_usage_reaction.kcat = find_max_kcat_mw(ec_code, kcat_max)
...
def remove_unused_metabolites(model):
    ...
    for metabolite in unused_metabolites:
        model.remove_metabolites([metabolite])
    print(f"Removed {len(unused_metabolites)} unused metabolites.")
